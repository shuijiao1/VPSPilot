...
SERVERS_JSON = Path(os.environ.get('JIAOOPS_INV', DATA_DIR / 'servers.json'))
...
BGP_OUT_ROOT = Path(os.environ.get('BGP_OUT_ROOT', MEDIA_DIR / 'jiaoops-bgp'))
IPPURE_TMP_ROOT = Path(os.environ.get('IPPURE_TMP_ROOT', TMP_DIR / 'jiaoops-ippure'))
...
def startup_check():
    ...
    if not ALLOWED_USERS:
        problems.append('ALLOWED_USERS is empty; JiaoOps requires whitelist mode')
...
def load_inventory() -> dict:
    if not SERVERS_JSON.exists():
        inv = {
            ...
            'defaults': {
                'ssh': {
                    'user': os.environ.get('JIAOOPS_DEFAULT_USER', 'root'),
                    'port': int(os.environ.get('JIAOOPS_DEFAULT_PORT', '22')),
                    'key': os.environ.get('JIAOOPS_DEFAULT_KEY', '/data/keys/id_ed25519'),
                }
            },
            'servers': [],
        }
...
async def send_report_images(bot, chat_id, report_links, prefix):
    if not report_links:
        return []
    out_dir = Path('/tmp/jiaoops-results')
...
def gb5_result_image(s, scores, out_png):
    ...
    d.text((70, 1255), 'Generated by JiaoOps · Geekbench 5', fill=muted, font=font_small)
...
async def health_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    code, out = await run_cmd(['python', '/app/jiaoops.py', 'health'], timeout=180)
...
async def export_config_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    path = TMP_DIR / f'jiaoops-export-redacted-{int(time.time())}.json'
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(text)
    with path.open('rb') as f:
        await update.message.reply_document(document=f, filename='jiaoops-servers-redacted.json', caption='已导出脱敏配置（密码已隐藏）。')
...
async def run_remote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    code, out = await run_cmd(['python', '/app/jiaoops.py', 'run', target, *cmd], timeout=120)
...
def main():
    ...
    print('jiaoops telegram bot started', flush=True)
